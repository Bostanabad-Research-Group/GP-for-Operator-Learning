...
class Model_DeepONet_NS(nn.Module):
...
class Network(nn.Module):
    def __init__(self, input_dim = 2, output_dim = 2, layers = [40 , 40 , 40 , 40 ], activation = 'tanh', encoder = 'cosine') -> None:
        ...
        activation_list = {'tanh':nn.Tanh(), 'Silu':nn.SiLU(), 'Sigmoid':nn.Sigmoid(), 'relu': nn.ReLU()}
...
class Network_CNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(128 * 5 * 5, 128)  # Adjust input size as per the feature map size after Conv layers
        self.relu3 = nn.ReLU()
    ...
    def forward(self, x):
        ...
        x = self.relu3(x)
        x = self.fc2(x)
        return x.repeat_interleave(29*29, dim=0)