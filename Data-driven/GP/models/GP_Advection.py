...
class GP_Advection(GPR):
    """ GP model for the Advection equation.
    Arguments:
    ...
    def __init__(
        ...
        X_data:torch.Tensor,
        U_data:torch.Tensor,
        kernel_phi:str = 'RBFKernel',
        kernel_y:str = 'RBFKernel',
        mean_type = 'zero',
        **tkwargs
    ) -> None:
        ...
        super(GP_Advection,self).__init__(
        ...
        if self.mean_type=='zero':
            ...
            branchnet = Network(input_dim = X_data.shape[1]-self.q, layers = [256,256,256,256], output_dim = 256, activation='relu')
            trunknet = Network(input_dim = self.q, layers = [256], output_dim = 256, activation='relu')
            setattr(self,'mean_module_NN_All', Model_DeepONet(trunknet, branchnet)) 
    ...
    def calculate_loss(self):
        ...
        if self.mean_type == 'zero':
            ...
            if self.mean_type == 'DeepONet':
                m_train_all = self.mean(self.train_inputs[0][:,:self.q], self.train_inputs[0][:,self.q:]) # DEEPONET
            elif self.mean_type == 'FNO':
                m_train_all = self.mean(self.phi_unique.unsqueeze(-1)).squeeze(-1).flatten() # FNO            
