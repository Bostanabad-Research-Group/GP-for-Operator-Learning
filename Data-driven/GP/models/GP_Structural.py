...
class GP_Structural(GPR):
    ...
    def __init__(
        ...
        **tkwargs
        ...
        self.mean_U_train = mean_U_train
        self.std_U_train = std_U_train
        self.q = 2
        ...
        super(GP_Structural, self).__init__(
        ...
        if self.mean_type=='zero':
            ...
            branchnet = Network(input_dim = X_data.shape[1]-self.q, layers = [64,64,64], output_dim = 64, activation='relu')
            trunknet = Network(input_dim = self.q, layers = [64,64,64], output_dim = 64, activation='relu')
    ...
    def calculate_loss(self):
        ...
        if self.mean_type == 'zero':
            ...
            if self.mean_type == 'DeepONet':
                ...
                m_train_all = self.mean(self.phi_unique.unsqueeze(1).repeat(1,41,1).unsqueeze(-1)).squeeze(-1).flatten()
    ...
    def fit(self, X_test, U_test_grid, num_iter:int = 1000) -> float:
        ...
        for j in epochs_iter:        
            ...
            rl2error_val = self.evaluate_error(self.X_val.detach(), self.U_val.detach())

            if rl2error_val < f_inc:
                current_state_dict = deepcopy(self.state_dict())
                f_inc = rl2error_val
            
            if j % 100 == 0:
                rl2error = self.evaluate_error(X_test.detach(), U_test_grid.detach())
    ...
    def evaluate_error(self, X_test_grid, U_test_grid):
        ...
        if self.mean_type == 'zero':
            ...
            if self.mean_type == 'DeepONet':
                ...
                m_train = self.mean(self.phi_unique.unsqueeze(1).repeat(1,41,1).unsqueeze(-1)).squeeze(-1).flatten()

            C_inv_offset = C.cholesky()._cholesky_solve(self.train_targets.unsqueeze(-1).to(torch.double) - m_train.unsqueeze(-1))
            
        for i in range(U_test_grid.shape[0]):
            U_test = U_test_grid[i,:].flatten()
            X_test = X_test_grid[i,...]
            ...
            if self.mean_type == 'zero':
                ...
                if self.mean_type == 'DeepONet':
                    m_col = self.mean(X_test[:,:self.q], X_test[:,self.q:]).squeeze(-1).flatten()

                elif self.mean_type == 'FNO':
                    m_col = self.mean(self.phi_unique.unsqueeze(1).repeat(1,41,1).unsqueeze(-1)).squeeze(-1).flatten()  

                C_inv_offset_mat = C_inv_offset.reshape(C_Phiphi.shape[0], -1).t()
                U_pred = m_col + (C_yY @ C_inv_offset_mat @ C_Phiphi).squeeze(-1)

            U_pred = U_pred * self.std_U_train + self.mean_U_train
